...
| `GET /api/v1/alerts/*` | Alert-system read surface: `overview`, `mechanisms[/{fingerprint}]`, `rules/{id}/instances`, `episodes[/{id}]`, `events`, `latest`, `deliveries[/{id}]`, `renders/{id}`, `ruleset`, `silences`, `health`. Separate `ALERTS_READ_API_KEY` scope — it does **not** fall back to the admin key. The full pipeline (capture → evaluate → plan → render → dispatch) exists; **live delivery is refused below rollout Stage 3** and requires evidence-gated promotion — the repo is committed at Stage 1, so nothing sends in production yet (`docs/ALERT_SYSTEM.md`) |
| `POST`/`DELETE /api/v1/alerts/silences[/{id}]` | Silence a rule, instance or bucket (`ALERTS_WRITE_API_KEY`; `Idempotency-Key` honoured) |
| `POST /api/v1/admin/alerts/evaluate` · `promote` · `recover` · `send-test` · `deliveries/{id}/retry` · `actionability` | Evaluate one captured sidecar (shadow by default); **evidence-gated** promotion (refused with machine-readable blockers under 409 when the committed replay evidence does not back the artifact); sweep stale leases; queue an audited memberless TEST delivery through the real dispatcher; manually retry an UNKNOWN delivery (`Idempotency-Key` + comment + `acknowledge_duplicate_risk=true`, new delivery, same generation, audit trail); record YES/NO/AMBIGUOUS actionability labels (X-API-Key) |
...
- **Alert system (mandate v3.2-FINAL; methodology unchanged; golden fixture byte-identical):** the event-driven, replayable alert layer that will replace the fixed daily digest — typed snapshot contract, immutable point-in-time sidecars, content-addressed rules (`v3.2.1`) + phrase registries (released `v3.2` frozen by digest; digest fragments in `v3.3`), pure evaluator with CAS all-or-nothing apply, per-ruleset continuation for open episodes, four-outcome sender contract on both transports (iMessage primary, 204-exact sipgate; a misconfigured transport **refuses** rather than falling back), non-P1 budgets with evidence-bound caps, a member-backed weekly digest with heartbeat-backed quiet-week liveness, standalone watchdog + host-side outage notifier, evidence-gated operator promotion (`register()` cannot promote; legacy ungated promotions are distrusted until re-promoted), a hard Stage-3 live-delivery floor (promotion is not a delivery switch; the sender is not even constructed below it), the audited admin surface (send-test / UNKNOWN retry / actionability), and a checkable Stage-4 cutover gate (`bubblegauge alerts cutover preflight`). Deterministic replay evidence is committed (`docs/alert-stage1-gate.json`): Stage 1 passes; **Stage 3 honestly fails** on the non-P1 volume caps (24h 5>3, 168h 8>6 on the coverage fixture) pending the operator's rule-tuning/cap decision. Live delivery stays off; the legacy daily digest keeps running until the observed Stage-4 gate.
