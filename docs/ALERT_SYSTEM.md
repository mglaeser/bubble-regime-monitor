...
`docs/alert-stage1-gate.json` is a replay of a synthetic history
(`tests/fixtures/alert_replay_history.json`, regenerated by
`python -m tests.fixtures.gen_alert_replay_history`) at stages 1 and 3. CI
...
The history is synthetic and says so — it exercises a hold → trim → de-risk →
recovery arc with two blind slots inside a firing episode. It establishes
regression coverage, **not** recall. Recall is a Stage 2 question and needs
`config/alert_mandatory_events.v3.2.json`, which ships empty on purpose:
inventing historical windows would manufacture a recall number nothing
measured, the same failure mode as inventing a `[PIN]`.
