...
def assert_no_trusted_claim(text: str) -> str:
    ...
    lowered = text.lower()
    for phrase in ("trusted review", "write-separated review",
        ...
        if phrase in lowered and "not " + phrase not in lowered:
            refuse(f"category=summary_claims_trust phrase={phrase!r} — this "
                   "panel is not write-separated and must not describe itself "
                   "as though it were")
    return text
