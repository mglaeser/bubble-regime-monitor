...
Trust boundaries: **(B1)** Internet→proxy→app (public reads); **(B2)** operator→admin (keyed writes); **(B3)** app→external data providers (egress); **(B4)** app→operator-configured hosted LLM gateway (computed numbers/enums plus a bounded prior LLM judgment in the digest; no external free text); **(B5)** app→sipgate (SMS to the operator's own number); **(B6)** developer→GitHub→CI→host (supply/delivery); **(B7)** app→imessage-proxy (iMessage to the operator's own handle; Bearer `messages:send` key, destination host supplied by CONFIG rather than fixed in code). B5 and B7 are alternatives, never both: exactly one transport carries the digest per run and there is no fallback.
...
| B4 | **Prompt injection / exfiltration** | Could untrusted content hijack the model or exfiltrate data? | External inputs are numbers/enums only. The digest may reuse a bounded prior model judgment, but the model has no tools/outbound channel and the result can affect only short digest text (**A-10/C-07/C-08/B-20 PASS/N-A**). A poisoned number or model output can perturb wording, not capabilities. |
| B4 | **Info disclosure (to provider)** | Prompt/data and an API key go to the configured HTTPS destination; a valid-but-wrong operator host is still a disclosure. The underlying serving provider/model and its retention policy are opaque. | HTTPS + no redirects/environment proxies + URL/header validation; no PII, secrets, user input, or scraped text in prompts (**C-34**). Residual: add a platform egress allowlist. |
| B4 | **DoS / resource exhaustion** | A peer can drip heartbeats, send oversized events, or tear a stream after partial text. | Monotonic deadline, inter-read timeout, line/event/wire/output bounds, terminal `response.completed` required; partial text discarded and callers degrade without a 500. One permanently stuck OS call can hold the single worker slot until process restart, but later callers still time out/degrade and no additional request workers or sockets accumulate. |
